  ...
  try {
    ...
    const { data, error } = await supabase.functions.invoke('create-subscription', {
      body: {
        plan_id: planId,
        user_id: user.id,
        // For Google Play Billing subscriptions, we can optionally attach the
        // purchase token and product ID so the Edge Function can immediately
        // activate the subscription and link it to the Google purchase.
        google_play_purchase_token: options?.googlePlayPurchaseToken,
        google_play_product_id: options?.googlePlayProductId,
      },
    });
...
/**
 * Verify a Google Play purchase and record it in the backend
 */
  ...
  try {
    ...
    if (error) {
      ...
      return {
        success: false,
        verified: false,
        error: error.message || 'Failed to verify Google Play purchase',
      };
    }

    const result = (data || {}) as { success?: boolean; verified?: boolean; error?: string };
    return {
    ...
    console.error('❌ Exception in verifyGooglePlayPurchase:', error);
    return {
      success: false,
      verified: false,
      error: error instanceof Error ? error.message : 'Failed to verify Google Play purchase',
    };
  }
};

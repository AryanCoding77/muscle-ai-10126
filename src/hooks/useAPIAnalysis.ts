    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (response.error.code === 'CANCELLED') {
          console.log('⏹️ Analysis cancelled by user (no error alert will be shown)');
          stateManager.setLoading(false, 'Analysis cancelled');
        } else {
      ...
      if (response.error && response.error.code === 'CANCELLED') {
        return null;
      }

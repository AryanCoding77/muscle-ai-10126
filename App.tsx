...
export default function App() {
  ...
  useEffect(() => {
    // Initialize Google Play Billing (IAP)
    const initIAP = async () => {
      try {
    ...
    const purchaseUpdateSubscription = RNIap.purchaseUpdatedListener(
      async (purchase) => {
        ...
        try {
          ...
          if (!user) {
            console.error('❌ No user found for purchase verification');
            return;
          }
    ...
    const purchaseErrorSubscription = RNIap.purchaseErrorListener(
      (error: any) => {
        ...
        if (error?.code && error.code !== 'E_USER_CANCELLED') {
  ...
  useEffect(() => {
    ...
    const handleDeepLink = (url: string) => {
      ...
      // Check if it's a payment success deep link
      if (url.includes('payment-success')) {
        console.log('✅ Payment success detected, navigating to homepage');
        
        // Wait for navigation to be ready, then navigate to home
        setTimeout(() => {
